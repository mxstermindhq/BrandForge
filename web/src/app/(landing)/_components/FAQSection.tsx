...
export function FAQSection() {
  ...
  return (
    <section className="px-4 py-20 sm:px-6 lg:px-8" style={{ background: "var(--color-bg)" }}>
      <div className="mx-auto max-w-4xl">
        <div className="mb-12 text-center">
          <h2 className="mb-4 font-headline text-4xl font-semibold text-[var(--color-text-primary)] sm:text-5xl">FAQ</h2>
          <p className="text-lg text-[var(--color-text-secondary)]">How BrandForge works</p>
        </div>

        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <div key={index} className="surface-card overflow-hidden rounded-xl border" style={{ borderColor: "var(--color-gold-border)", background: "var(--color-surface)" }}>
              <button
                type="button"
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className="flex w-full items-center justify-between p-6 text-left transition-colors hover:bg-[var(--color-surface-2)]"
              >
                <span className="pr-4 text-lg font-semibold text-[var(--color-text-primary)]">{faq.question}</span>
                <span className="material-symbols-outlined shrink-0 text-[var(--color-text-secondary)]">
                ...
                {openIndex === index ? (
                  ...
                  >
                    <div className="px-6 pb-6">
                      <p className="text-left leading-relaxed text-[var(--color-text-secondary)]">{faq.answer}</p>
                    </div>
                  </motion.div>
                ) : null}
              </AnimatePresence>
